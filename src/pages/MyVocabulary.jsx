...
import { 
  ...
  MdAutorenew, MdShuffle, MdStopCircle, MdStop
} from 'react-icons/md';
...
const MyVocabulary = () => {
  ...
  const handlePlayWithOrder = (newOrder) => {
    if (isAutoPlaying && playOrder === newOrder) {
      stopCurrentPlay();
      setIsAutoPlaying(false);
    } else {
  ...
  return (
    <div className="page-container fadeIn">
      ...
      <div className="vocab-controls-panel">
          ...
          <div className="filter-controls-group">
            ...
            <div className="word-count-badge">
              {selectedFolder === 'All Folders' 
                ? `全部: ${filteredVocab.length}` 
                : `${filteredVocab.length} 個單字`}
            </div>
            
            {selectedFolder !== 'All Folders' && (
              <button 
                className="delete-folder-minimal" 
        ...
        <div className="function-panel-card single-row">
          <div className="play-controls-row">
            <button 
              className={`play-order-btn ${isAutoPlaying && playOrder === 'sequential' ? 'active' : ''}`}
              onClick={() => handlePlayWithOrder('sequential')}
              disabled={filteredVocab.length === 0}
            >
              {isAutoPlaying && playOrder === 'sequential' ? <MdStop size={22} /> : <MdPlayCircleOutline size={22} />}
              <span>順序播放</span>
            </button>

            <button 
              className={`play-order-btn ${isAutoPlaying && playOrder === 'random' ? 'active' : ''}`}
              onClick={() => handlePlayWithOrder('random')}
              disabled={filteredVocab.length === 0}
            >
              {isAutoPlaying && playOrder === 'random' ? <MdStop size={22} /> : <MdShuffle size={22} />}
              <span>隨機播放</span>
            </button>

            <div className="vertical-divider"></div>

            <div className="inline-setting-v2 mode-setting">
            ...
            <div className="inline-setting-v2 accent-setting">
            ...
            <div className="inline-setting-v2 speed-setting">
                  ...
                  <>
                    <div className="vocab-card-header">
                      ...
                      <div className="card-actions-top">
                        <button className={`play-btn-small ${playingId === item.id ? 'active playing' : ''}`} onClick={(e) => { e.stopPropagation(); handlePlayCard(item); }}>
                          {playingId === item.id ? <MdStopCircle size={18} /> : <MdVolumeUp size={18} />}
                        </button>
                      </div>
                    </div>
